...
def main(input_image):
    ...
    output_raw = "pd120_11025Hz_script.raw"
    ...
    # 重采样并保存 RAW
    print(f"[*] 正在重采样至 {TARGET_SR_RAW}Hz...")
    resampled = librosa.resample(waveform, orig_sr=SAMPLE_RATE, target_sr=TARGET_SR_RAW)
    
    # 末尾填充
    resampled_padded = np.pad(resampled, (0, 2048), mode='constant')
    
    print(f"[+] 正在保存 RAW (Float32): {output_raw}")
    with open(output_raw, 'wb') as f:
        f.write(resampled_padded.astype(np.float32).tobytes())
    
    print("[!] 完成！")

if __name__ == "__main__":
    import sys
    # 默认寻找当前目录下的 input.png，或者通过命令行输入
